% Source-target evaluations on the unit square, Laplace kernel.
%
% This example computes the interactions between random source and target points
% on the unit square via the Laplace kernel. The associated matrix is
...
function mv_square1(M,N,occ,p,rank_or_tol,Tmax,near,store)
  ...
  if nargin < 5 || isempty(rank_or_tol), rank_or_tol = 1e-9; end
