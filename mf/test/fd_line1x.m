% Three-point stencil on the unit line, constant-coefficient Poisson equation,
...
function fd_line1x(n,occ,symm,doiter,diagmode)
  ...
  idx = zeros(n+1,1);  % index mapping to each point, including "ghost" points
  idx(2:n) = 1:N;
  ...
  % interactions with ...
  Jl = idx(lft); Sl = -e;          % ... left
  Jr = idx(rgt); Sr = -e;          % ... right
  Jm = idx(mid); Sm = -(Sl + Sr);  % ... middle (self)
  ...
  A = sparse(I,J,S,N,N);
  clear idx Jl Sl Jr Sr Jm Sm I J S
