...
function fd_square(n,occ,rank_or_tol,symm,doiter)
  ...
  t = toc;
  mem = whos('FS.L').bytes/1e6;
  if dolu, mem = mem + (whos('FS.U').bytes + whos('FS.P').bytes)/1e6;
  else,    mem = mem + (whos('FS.D').bytes + whos('FS.P').bytes)/1e6;
  end
  fprintf('rskel_xsp factor time/mem: %10.4e (s) / %6.2f (MB)\n',t,mem)
