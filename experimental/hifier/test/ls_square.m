...
function ls_square(m,n,delta,occ,p,rank_or_tol,skip,rdpiv,store)
  ...
  F = hifie2r(Afun,rx,cx,occ,rank_or_tol,pxyfun,opts);
  w = whos('F');
  fprintf([repmat('-',1,80) '\n'])
  fprintf('mem: %6.2f (MB)\n',w.bytes/1e6)
  ...
  if ~isoctave()
    ...
    tic
    [Z,~,~,piter] = gmres(@(x)(hifier_sv(F,ifmm_mv(G,x,Afun))),Y,[],1e-9,32);
    t = toc;
    fprintf('cg/gmres')
  end
  e1 = norm(Z - Y)/norm(Z);
