...
function Y = hifde_sv(F,X,trans)
  ...
  if strcmpi(trans,'t'), Y = conj(hifde_sv(F,conj(X),'c')); return; end

  % dispatch to eliminate overhead
  if strcmpi(F.symm,'n')
    if strcmpi(trans,'n'), Y = hifde_sv_nn(F,X);
    else,                  Y = hifde_sv_nc(F,X);
    end
  elseif strcmpi(F.symm,'s')
    if strcmpi(trans,'n'), Y = hifde_sv_sn(F,X);
    else,                  Y = hifde_sv_sc(F,X);
    end
  elseif strcmpi(F.symm,'h'), Y = hifde_sv_h(F,X);
  elseif strcmpi(F.symm,'p'), Y = hifde_sv_p(F,X);
  end
end