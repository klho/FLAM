...
function Y = hifde_mv(F,X,trans)
  ...
  if strcmpi(trans,'t'), Y = conj(hifde_mv(F,conj(X),'c')); return; end

  % dispatch to eliminate overhead
  if strcmpi(F.symm,'n')
    if strcmpi(trans,'n'), Y = hifde_mv_nn(F,X);
    else,                  Y = hifde_mv_nc(F,X);
    end
  elseif strcmpi(F.symm,'s')
    if strcmpi(trans,'n'), Y = hifde_mv_sn(F,X);
    else,                  Y = hifde_mv_sc(F,X);
    end
  elseif strcmpi(F.symm,'h'), Y = hifde_mv_h(F,X);
  elseif strcmpi(F.symm,'p'), Y = hifde_mv_p(F,X);
  end
end