...
%    this may not be too practical; see HIFDE_SPDIAG for an alternative.
%
...
%
%    D = HIFDE_DIAG(F,DINV,OPTS) also passes various options to the algorithm.
...
function D = hifde_diag(F,dinv,opts)
  ...
  for lvl = 1:nlvl-1  % current level is lvl+1
    ...
    for i = F.lvp(lvl)+1:F.lvp(lvl+1)
      ...
      I_ = [I_(rem(I_)); J_(rem(J_))];
      ex = I_(~ismemb(unique(I_),sort([rd sk])));
      [I_,J_] = ndgrid(ex,sk);
      I__ = I_(:);
      J__ = J_(:);
      I_ = [I__; J__];
