...
function ie_square3(n,k,occ,p,rank_or_tol,symm)
  ...
  h = 1/n;
  intgrl = 4*dblquad(@(x,y)(0.25i*besselh(0,1,k*sqrt(x.^2 + y.^2))), ...
                     0,h/2,0,h/2);
  ...
  rskelf_mv(F,X);
  t = toc;
  [e,niter] = snorm(N,@(x)(mv (x) - rskelf_mv(F,x,'n')), ...
  ...
  Y = rskelf_sv(F,X);
  t = toc;
  [e,niter] = snorm(N,@(x)(x - mv (rskelf_sv(F,x,'n'))), ...
  ...
  % FFT multiplication
  function y = mv(x)
    y = ifft2(G.*fft2(reshape(sqrtb.*x,n,n),2*n-1,2*n-1));
    y = sqrtb.*reshape(y(1:n,1:n),N,1);
    y = y + x;
  end

  % FFT adjoint multiplication
  function y = mva(x)
    y = conj(mv(conj(x)));
  end
end