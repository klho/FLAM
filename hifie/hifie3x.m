% HIFIE3X       Hierarchical interpolative factorization for integral operators
%               in 3D with accuracy optimizations for second-kind integral
%               equations.
%
...
function F = hifie3x(A,x,occ,rank_or_tol,pxyfun,opts)
  ...
  if opts.skip < 0
    error('FLAM:hifie3x:negativeSkip','Skip parameter must be nonnegative.')
  end
  if ~(strcmp(opts.symm,'n') || strcmp(opts.symm,'s') || strcmp(opts.symm,'h'))
    error('FLAM:hifie3x:invalidSymm', ...
