...
%
% This example solves the Dirichlet Laplace problem on the unit ball by using a
% double-layer potential, which yields a second-kind boundary integral equation
% on the unit sphere, represented using flat triangles. The system is
% discretized via collocation with tensor-product Gauss-Legendre integration for
% the near-field panel quadratures (within a distance of approximately one
% triangle size) and simple point-to-point interactions for the far-field ones.
% The resulting matrix is square, real, and unsymmetric.
%
